...
//!
//! Everything lives on the device, in plain files inside the app's private
//! data directory (`RIDDLE_DATA_DIR`, supplied by the activity) under
//! `memories/`:
//!
...
//!
//! Deleting the folder (or "Forget everything" in settings) makes the diary
//! forget. `RIDDLE_MEMORY=off` disables remembering entirely (no storage, no
//! context sent with requests).
//!
//! **Android port note.** Upstream defaulted to `/home/root/riddle-data`,
//! a path that does not exist inside an APK's sandbox; the location now comes
//! from settings, defaulting to `memories/` relative to the data directory.
...
impl MemoryStore {
    ...
    pub fn open() -> Option<Self> {
        match config::var("RIDDLE_MEMORY").as_deref() {
            Some("off") | Some("0") | Some("no") | Some("false") => return None,
            _ => {}
        }
        let dir = match config::var("RIDDLE_MEMORY_DIR") {
            Some(d) => PathBuf::from(d),
            None => {
                // Android's app-private storage, handed to us by the activity.
                let base = config::var("RIDDLE_DATA_DIR")?;
                PathBuf::from(base).join("memories")
            }
        };
    ...
    /// Make the diary forget everything. This is the settings screen's
    /// "Forget everything": the index and every stroke file are removed, so
    ...
    fn rewrite_index(&mut self) {
        ...
        if let Err(e) = std::fs::write(self.index_path(), out) {
            eprintln!("riddle: memory write failed: {e}");
        }
    }
