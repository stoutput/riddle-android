...
/// Android: the diary is an ordinary app here, so the takeover lines about
/// leaving with five fingers, the power button, and `xochitl` would all be
/// wrong. The gestures listed are the ones the Android view implements.
const BODY_ANDROID: &[&str] = &[
    "Write, then rest your quill:",
    "the diary drinks your ink and Tom replies.",
    "",
    "The diary remembers. Ask it:",
    ...
    "and the page will rise again.",
    "",
    "Use the eraser tip, or the",
    "eraser in the toolbar, to rub ink out.",
    "",
    "A large ? summons this guide.",
    "Hold the pen still to open settings.",
];
...
/// Which gesture list the guide shows. Upstream's `takeover: bool` becomes an
/// explicit mode because Android is a third case, not a variant of either.
#[derive(Clone, Copy, PartialEq, Eq)]
#[allow(dead_code)] // Takeover/Windowed are upstream's reMarkable modes
pub enum Mode {
    /// Full takeover on the tablet: riddle owns touch and the power button.
    Takeover,
    /// Windowed inside the reMarkable UI via AppLoad/QtFB.
    Windowed,
    /// An ordinary Android app window.
    Android,
}
...
pub fn show(surf: &mut Surface, font: &FontRef, mode: Mode) -> Help {
    let body = match mode {
        Mode::Takeover => BODY_TAKEOVER,
        Mode::Windowed => BODY_WINDOWED,
        Mode::Android => BODY_ANDROID,
    };
...
///
/// Unused on Android: the sleep card belonged to takeover mode, where the
/// diary grabbed the power button and suspended the tablet itself. An app
/// window is backgrounded by the system instead, so there is no page to show.
#[allow(dead_code)]
...
mod tests {
    ...
    fn modal_renders_and_restores() {
        ...
        let panel = show(&mut surf, &font, Mode::Takeover);
